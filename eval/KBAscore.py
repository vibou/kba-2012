...
def full_run_metrics(CM, Scores, macro=False):
    '''
    ...
    macro, bool, false=average over urlnames, true=average over each document
...
def score_confusion_matrix (path_to_run_file, annotation, cutoff_step, unannotated_is_TN, include_training):
    '''
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--use-macro-averaging', default=False, action='store_true', dest='macro_is_true',
        help='compute scores for each urlname individually and then average.  Default is micro averaging, i.e. average across all (stream_id, urlname) pairs equally regardless of lumpiness in num_stream_ids per urlname.')
    parser.add_argument(
    ...
    annotation = load_annotation(args.annotation, args.include_relevant, args.include_neutral)
    ...
    for run_file in os.listdir(args.run_dir):
        ...
        (CM['average'], Scores['average']) = full_run_metrics(CM, Scores, args.macro_is_true)
        ...
        for urlname in Scores:
            teamscores[run_file_name][urlname]['F'] = max([Scores[urlname][cutoff]['F'] for cutoff in Scores[urlname]])
            teamscores[run_file_name][urlname]['SU'] = max([Scores[urlname][cutoff]['SU'] for cutoff in Scores[urlname]])
